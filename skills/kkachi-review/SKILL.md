...
V02FLOW-013 review focus: verify that `implementation_goal_bundle_ready is goal-bundle-only and never sufficient for implementation completion`. For `impl`, `test-enhance`, `ai-slop-cleaner`, `optimize`, and `docs-update`, reviewers must find `implementation_diff_ready` or `implementation_verified` executor-loop evidence fields before accepting closeout: `changed_source_refs`, `diff_refs`, `checkpoint_ref`, `checkpoint_status`, `verification_output_refs`, `checksums`, `termination_reason`, `HOME`, and `no_authority_boundaries`. Goal-bundle-only wording, stale verification, missing executor-loop refs, or Blue/color source-patch fallback without a recorded exception is a request-changes finding.
