...
6. For `development` / implementation tasks, run KAS MAR as the default independent review lane after the first color review and feedback handling unless 주군 explicitly waives or replaces it before start. MAR review completion is role-first: required roles are `logic`, `security`, `arch`, `cve`, and `test_adequacy`; unresolved required role coverage fails closed and must be reported rather than replaced by undeclared providers. After MAR feedback is triaged/applied/rejected, perform any required Blue + Red/Orange/Gray post-change re-review before final/pre-commit reporting.
