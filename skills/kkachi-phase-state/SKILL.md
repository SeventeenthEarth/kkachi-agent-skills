...
`phase-plan.yaml` is KHS-declared run-local execution state stored at `.kkachi/runs/<run_id>/phase-plan.yaml` through `phase-plan init/show/set/validate`. `.kkachi-workflow.yaml` is project-level graph state only when the same effective KAH binary proves `kkachi-agent-helper graph` support and validates/explains or applies it. Current KAH (with graph-required phase support) initializes and validates phase-plan rows from baseline KHS phases plus graph-declared `required: true` phases, so project workflows can machine-enforce additional rows. KHS policy requires GLM Octo for `development` / implementation tasks unless 주군 explicitly waives Octo before start, normally represented as later request-feedback/handle-feedback rows after first color review. For non-implementation durable-change runs, GLM Octo is project/request/high-risk opt-in policy; omit the GLM Octo phases/gates from `.kkachi-workflow.yaml` or declare them optional and mark them `not_applicable` with a reason when not applicable. KAH must not require Octo globally for all task classes.

KHS review policy is broader than implementation-only work: for any active KAS/KAH run that changes durable repository artifacts, workflow state, docs, or release/commit evidence, first Blue + Red/Orange/Gray color review should be represented in the phase plan and evidence artifacts even when the task class is `docs_only`, `research_evidence`, `bootstrap_config`, or `collaboration_review`. Pure read-only explanations and bounded command reports should usually stay outside KAS/KAH; if a run exists and no durable artifact changed, review phases may be `not_applicable` only with a concrete reason. If the task class is `development` / implementation, or Octo is explicitly requested/project-opted-in/high-risk opted-in for another task class, post-Octo feedback handling plus a second Blue + Red/Orange/Gray re-review become mandatory before final/pre-commit reporting.
