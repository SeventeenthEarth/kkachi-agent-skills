...
8. Invoke `kkachi-backend-select` only when a KAB-backed lane is selected or bridge evidence is claimed:
   - Stage 1 direct Codex SDK/app-server runner baseline records runner metadata, `thread_id`, prompt/output paths, and no-KAB-Codex rationale instead via `templates/runners/direct-codex-sdk-appserver-runner.py.tmpl` (`openai_codex` -> SDK-managed `codex app-server --listen stdio://`; not `codex exec`, generic `openai` SDK output, raw app-server transport, or KAB `native_codex` evidence)
