...
V02FLOW-009 routes task-bound `docs-update` mutation through the selected GJC `ultragoal` executor lane by default after bounded approval when the run is active KAS/KAH roadmap-task work. The docs-update phase brief must include accepted-scope refs, ralplan/hash, Blue approval, changed-doc bounds, preservation locks, docs authority/non-goals, verification commands, and selected ultragoal session/goal refs at dispatch time. Missing KAH V02FLOW-010 capability/readback, stale refs, unsafe refs/checksums, absent verification, or native GJC `ai-slop-cleaner` or `remove-ai-slop` requests fail closed; they do not authorize Blue/color source-patch fallback.

V02FLOW-013/V02FLOW-015 tighten `docs-update` closeout: `implementation_goal_bundle_ready is goal-bundle-only and never sufficient for implementation completion`. `implementation_diff_ready` and `implementation_verified` require executor-loop evidence before this mutation-capable phase can close, including selected executor lane `gjc_ultragoal_executor_loop_candidate`, phase/canonical phase, argv/process refs, real-user `HOME`, cwd, timestamps, exit code, `changed_source_refs`, non-empty `diff_refs`, `checkpoint_ref`, `checkpoint_status`, `verification_output_refs`, checksums, repeat/termination reason, and `no_authority_boundaries`. Goal-bundle-only evidence, stale verification, missing executor-loop refs, unsupported-capability warnings, or authority overclaims must fail closed; docs authority decisions, review/MAR disposition, final, commit, install, runtime, and publication remain separate gates.
