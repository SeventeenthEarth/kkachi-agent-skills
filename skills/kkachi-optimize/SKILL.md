...
1. AI slop cleanup: ask the selected implementer lane to remove AI-ish artifacts, overexplaining comments, speculative branches, noisy abstractions, ungrounded text, and awkward generated prose. In Stage 1 that lane is direct Codex app-server; in Stage 2 it is KAB Codex-first; in Stage 3 it is the selected KAB backend. Run `make test` after this stage if files changed.
