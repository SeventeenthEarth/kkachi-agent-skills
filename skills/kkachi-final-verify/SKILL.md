...
- V02FLOW-016 release-evidence proof must include effective KAH `capabilities --json` readback for `gjc_executor_loop_evidence=true`, `diagnostics_deferred_feedback=true`, `mar_legacy_rejection_diagnostics=true`, `mar_provider_adapter_safety=true`, and `mar_migration_diagnostics=false`; GJC execution evidence must show `HOME=/Users/draccoon`, any future KAH MAR provider execution must source HOME from `toolchain.operator.real_user_home`, deferred-feedback diagnostics must be preserved, legacy Python MAR must be absent/rejected input only, provider MAR execution must not be claimed before the official v0.2.1 path, and waiver-only second-color adoption is N/A.
...
Final reports to 주군 must enforce `docs/sot/stage-report-contract.md`. Before claiming task-classification, plan-stage, implementation-stage, or review-stage completion, verify the report explains functional change, old behavior/reason, document direction, plan drift and color-vet handling, implementation before/after behavior, step/test coverage, color/MAR/second-color feedback, feedback fixes, every deferred item with a short what/why/future-owner reason, and the remaining release/install/commit boundary. Reject file-list-only or `stage done` summaries as incomplete.
