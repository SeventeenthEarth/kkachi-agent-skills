...
5. For active KAS/KAH roadmap-task work on the Codex app-server lane, include the actor boundary: Codex app-server owns the plan draft/revision and substantive code/test/build/task-doc mutations; Blue/Red vet the returned plan and approve or request changes; Blue/Red/Orange/Gray review and verify rather than directly editing, unless 주군 explicitly requested a direct role edit or the work is outside the roadmap/KAS+KAH path.
6. For planner prompts, explicitly say plan-only: Codex drafts the plan and waits; Blue and Red vet it; any requested plan change must come back as a Codex revised plan before implementation starts.
7. For implementation or feedback-handling prompts, explicitly say color-review and GLM-review fixes must be applied by Codex app-server/KAB implementer, not by Blue/Red/Orange/Gray direct patches.
