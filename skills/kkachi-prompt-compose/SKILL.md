...
For V02FLOW-009 implementer and remediation prompts, render phase briefs from `registries/phase-contracts.yaml#v02flow009_implementer_phase_dispatch_contract`: task/run id, approved scope, accepted-scope ref, ralplan artifact ref/hash, Blue plan-lock or implementation approval ref, selected ultragoal session/goal refs at dispatch time, changed-surface bounds, preservation locks, non-goals, expected evidence, verification commands, and stop/block conditions. Unknown aliases, unsupported dispatch shapes, missing KAH V02FLOW-010 capability/readback, stale refs, unsafe checksums, stale verification, and native GJC `ai-slop-cleaner` or `remove-ai-slop` requests fail closed. The selected GJC `ultragoal` executor lane is the default mutation owner; Blue/color direct patches require a recorded exception.
