...
func TestRootVersionExitsZeroAndPrintsVersion(t *testing.T) {
	for _, args := range [][]string{{"--version"}, {"-version"}, {"version"}} {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			if out != "kkachi-agent-skills v0.2.5" {
...
func TestVersionJSONIncludesNameAndVersionOnly(t *testing.T) {
	for _, args := range [][]string{{"version", "--json"}, {"--version", "--json"}} {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			if len(payload) != 2 || payload["name"] != "kkachi-agent-skills" || payload["version"] != "0.2.5" {
...
func TestDoctorWorkflowGraphJSONAndFlagValidation(t *testing.T) {
	...
	if payload["kas"].(map[string]any)["cli_version"] != "0.2.5" || payload["kah"].(map[string]any)["graph_help_state"] != "ok" {
