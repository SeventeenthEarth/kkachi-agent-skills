...
func TestRootVersionExitsZeroAndPrintsVersion(t *testing.T) {
	for _, args := range [][]string{{"--version"}, {"-version"}, {"version"}} {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			if out != "kkachi-agent-skills 0.1.8" {
...
func TestVersionJSONIncludesBuildMetadata(t *testing.T) {
	...
	if payload["ok"] != true || payload["command"] != "version" || payload["cli_version"] != "0.1.8" {
...
func TestDoctorWorkflowGraphJSONAndFlagValidation(t *testing.T) {
	...
	if payload["kas"].(map[string]any)["cli_version"] != "0.1.8" || payload["kah"].(map[string]any)["graph_help_state"] != "ok" {
