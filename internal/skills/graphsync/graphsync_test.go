...
func newFakeRunner(project string) *fakeRunner {
	return &fakeRunner{
		...
		results: map[string]doctor.CommandResult{
			...
			"graph diff --from .kkachi-workflow.yaml --to .kkachi/graph/candidates/kas-default-9e933dd77ff2c95c.yaml --semantic --json": {Stdout: []byte(`{"ok":true,"summary":"semantic diff ready","risk_flags":["phase_path_change"],"reason_codes":["graph_stale"]}`)},
			"graph propose --candidate-file .kkachi/graph/candidates/kas-default-9e933dd77ff2c95c.yaml --reason repair --json":          {Stdout: []byte(`{"ok":true,"proposal_id":"prop-1","proposal_path":".kkachi/graph/proposals/prop-1.yaml","approval_required":true,"risk_flags":["phase_path_change"],"reason_codes":["proposal_recorded"]}`)},
