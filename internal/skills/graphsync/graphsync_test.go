...
func TestDefaultRunnerUsesRepoToolchainKAH(t *testing.T) {
	...
	project := t.TempDir()
	helper := writeGraphsyncHelper(t, filepath.Join(project, "toolchain", "kkachi-agent-helper"), "kkachi-agent-helper 0.1.12")
	writeGraphsyncToolchain(t, project, "kah_cli: v0.1.12\nkah_cli_path: "+helper+"\n")
	pathDir := t.TempDir()
	writeGraphsyncHelper(t, filepath.Join(pathDir, "kkachi-agent-helper"), "kkachi-agent-helper 0.1.9")
	t.Setenv("PATH", pathDir)

	result := commandRunner{}.Run(project, "--version")
	if result.Err != nil {
		t.Fatalf("default graphsync runner failed: %v\nstderr=%s", result.Err, result.Stderr)
	}
	if got := strings.TrimSpace(string(result.Stdout)); got != "kkachi-agent-helper 0.1.12" {
		t.Fatalf("default graphsync runner used %q, want repo toolchain helper", got)
	}
}

func writeGraphsyncToolchain(t *testing.T, project, content string) {
	t.Helper()
...
func writeGraphsyncHelper(t *testing.T, path, version string) string {
	t.Helper()
...
func newFakeRunner(project string) *fakeRunner {
	return &fakeRunner{
		...
		results: map[string]doctor.CommandResult{
			...
			"graph diff --from .kkachi-workflow.yaml --to .kkachi/graph/candidates/kas-default-55c4db299aa44d64.yaml --semantic --json": {Stdout: []byte(`{"ok":true,"summary":"semantic diff ready","risk_flags":["phase_path_change"],"reason_codes":["graph_stale"]}`)},
			"graph propose --candidate-file .kkachi/graph/candidates/kas-default-55c4db299aa44d64.yaml --reason repair --json":          {Stdout: []byte(`{"ok":true,"proposal_id":"prop-1","proposal_path":".kkachi/graph/proposals/prop-1.yaml","approval_required":true,"risk_flags":["phase_path_change"],"reason_codes":["proposal_recorded"]}`)},
...
func TestProposeCommandOrderForStaleGraph(t *testing.T) {
	...
	want := []string{
		...
		"capabilities --json",
		"graph --help",
		"graph diff --from .kkachi-workflow.yaml --to .kkachi/graph/candidates/kas-default-55c4db299aa44d64.yaml --semantic --json",
		"graph propose --candidate-file .kkachi/graph/candidates/kas-default-55c4db299aa44d64.yaml --reason repair --json",
	}
