...
func TestCommandRunnerUsesRepoToolchainKAH(t *testing.T) {
	...
	project := t.TempDir()
	helper := writeWorkflowCreatorHelper(t, filepath.Join(project, "toolchain", "kkachi-agent-helper"), "kkachi-agent-helper 0.1.12")
	writeWorkflowCreatorToolchain(t, project, "kah_cli: v0.1.12\nkah_cli_path: "+helper+"\n")
	pathDir := t.TempDir()
