...
- KASREL TODO SOT color review loop: Round 1 `red=t_0c91b4cf`, `orange=t_cedd82fc`, `gray=t_983ce96e`; Round 2 final consensus `red=t_8a90c4ef`, `orange=t_44fa2173`, `gray=t_3e1024bf`; blue final synthesis recorded on all six cards
...
> Goal: make KAS resilient to Hermes release changes in skill source layout, bundled/optional skill movement, installed-profile drift, and profile-local shadowing. This epic starts from the color-reviewed TODO SOT and turns it into a staged release-compatibility plan. The roadmap entry itself is not authorization to rebuild/install the KAS binary, repair profile skills, mutate auth/tokens/gateway/provider/model config, or activate KAB runtime behavior.

| Task ID | Title | Status | Acceptance criteria | Evidence and review gates |
|---|---|---|---|---|
| KASREL-001 | Specify Hermes v0.16 skill provenance and dependency audit contract | Planned | Promote or refine `docs/sot/TODO-kasrel-hermes-v016-provenance-plan.md` into the accepted release-compatibility contract. The contract defines canonical source classes, path/manifest-based classification, deleted-bundle cleanup/fail-closed diagnostics, the distinction between skill dependencies and command-surface dependencies, and future JSON output fields for `list`, `install --dry-run`, and `doctor` without authorizing profile mutation or KAB activation. | Existing TODO SOT evidence plus Round 2 color consensus (`red=t_8a90c4ef`, `orange=t_44fa2173`, `gray=t_3e1024bf`), responsible-approver confirmation, readback, targeted no-fallback/stale-source search, `git diff --check`, and docs index/roadmap update evidence. |
| KASREL-002 | Implement skill inventory and provenance classification | Planned | KAS builds a profile skill inventory from profile-local skills, KAS manifest data, hub-installed metadata, bundle paths, and configured external/ops skill directories; JSON outputs expose provenance evidence without secrets; name shadowing is reported deterministically; approved install behavior remains profile-local and approval-gated. | Unit tests for path classification and shadowing, isolated temp-profile CLI smoke tests, representative real-profile no-write `--dry-run --json` or `doctor --json` evidence, `make test`, color review, and commit approval. |
...
- Hermes release compatibility, skill provenance, shadowing, and dependency audit work is now tracked by KASREL; `docs/sot/TODO-kasrel-hermes-v016-provenance-plan.md` is the color-reviewed planning basis until KASREL-001 accepts or refines it into a final contract.
- KAB alignment requires a separately assigned KAB docs/update task.

## Next record action

INITDOC, BOOTSTRAP, `CLIMVP-001` through `CLIMVP-005`, `GRAPHMVP-001` through `GRAPHMVP-004`, `STALECLEAN-001` through `STALECLEAN-004`, `KABADOPT-001` through `KABADOPT-003`, and KASUPD-001 through KASUPD-003 are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. KABADOPT-004 remains `Planned` until its own Stage 2 pilot evidence/review gates pass. KASUPD-004 remains `Planned` until its own write-capable sync/pilot evidence gates pass. KASREL-001 through KASREL-004 remain `Planned` until their release-compatibility contract, provenance implementation, dependency audit, and skill-guidance evidence gates pass. No further STALECLEAN task is listed; any epic-level closure needs a dedicated review/update task.
