...
INITDOC, BOOTSTRAP, `CLIMVP-001` through `CLIMVP-005`, `GRAPHMVP-001` through `GRAPHMVP-004`, and `STALECLEAN-001` are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. `STALECLEAN-002` is the next listed planned cleanup item; actual implementation order remains governed by the roadmap and assigned scope.
