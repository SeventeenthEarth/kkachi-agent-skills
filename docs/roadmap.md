...
INITDOC, BOOTSTRAP, `CLIMVP-001` through `CLIMVP-005`, `GRAPHMVP-001` through `GRAPHMVP-004`, `STALECLEAN-001` through `STALECLEAN-004`, and `KABADOPT-002` are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. KABADOPT-001, KABADOPT-003, and KABADOPT-004 remain `Planned` until their own evidence/review gates pass. No further STALECLEAN task is listed; any epic-level closure statement still requires the responsible review/closure gate rather than this roadmap line alone.
