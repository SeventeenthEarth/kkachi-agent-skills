...
INITDOC and BOOTSTRAP are closed. `CLIMVP-001`, `CLIMVP-002`, and `CLIMVP-003` are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. The next work item is `CLIMVP-004`: implement approved copy install with manifest/checksum and recovery unless 주군 chooses a different implementation slice.
