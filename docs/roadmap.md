...
INITDOC, BOOTSTRAP, `CLIMVP-001` through `CLIMVP-005`, `GRAPHMVP-001` through `GRAPHMVP-004`, and `STALECLEAN-001` through `STALECLEAN-003` are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. `STALECLEAN-004` is the next listed planned cleanup item; actual implementation order remains governed by the roadmap and assigned scope.
