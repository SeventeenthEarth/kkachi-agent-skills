...
| Task ID | Title | Status | Acceptance criteria | Evidence and review gates |
|---|---|---|---|---|
| KASREL-001 | Specify Hermes v0.16 skill provenance and dependency audit contract | Completed | `docs/sot/kasrel-hermes-v016-provenance-contract.md` is the accepted docs/spec-only release-compatibility contract; KASREL-002/003/004 implementation and guidance work remain planned. | Round 2 color consensus (`red=t_8a90c4ef`, `orange=t_44fa2173`, `gray=t_3e1024bf`), responsible-approver confirmation, accepted SOT readback, targeted no-fallback/stale-source search, docs-contract test, `git diff --check`, and docs index/roadmap update evidence. |
...
| KASREL-003 | Implement KAS dependency audit | In Progress | Implementation evidence exists for typed skill and command-surface dependency audit records in `list --json`, `install --dry-run --json`, and `doctor --json`; KAS pack dependencies are separated from external skill names and command-surface dependencies; KAS-internal phase-skill handoffs are checked against source/profile install evidence; Hermes/KAH/Kanban/KAB command surfaces are reported as command-surface dependencies rather than fake skill dependencies; deleted-bundle references remain fail-closed cleanup diagnostics. Do not mark Completed until final review/KAH final gate evidence is complete. | Current KAH run evidence: `run-20260608T135736Z-12a865a6fc30/impl-log.md`, `codex-implement-output.md`, `codex-fix-1-output.md`, `make-test-after-fix-1.log`, and `smoke3-summary.txt`; evidence reports focused unit/doctor/install coverage, `git diff --check`, `HOME=/Users/draccoon make test`, and no-write smoke with unchanged profile manifest. Pending gates: final review/color review, KAH final gate, and commit approval; no profile/KAB/auth mutation without separate approval. |
| KASREL-004 | Update KAS skill guidance and final gates | Planned | KAS guidance that affects install/readiness/orchestration/review/final verification requires provenance/dependency evidence before confidence claims, preserves no-fallback handling for deleted bundle skills, and keeps KAS/KAH/KAB responsibilities distinct. Likely surfaces include `kkachi-install-guide`, `kkachi-final-verify`, `kkachi-orchestrate`, and `kkachi-review`. | Readback of changed guidance, targeted search for forbidden fallback wording, relevant docs-contract or repository test target plus `make test` where affected, color review, final verification evidence, and commit approval. |
...
- Hermes release compatibility, skill provenance, shadowing, and dependency audit vocabulary are now defined by accepted SOT `docs/sot/kasrel-hermes-v016-provenance-contract.md`; KASREL-002 is completed/pre-commit-ready after the in-tree provenance inventory pass, Gray fix, official KAB GLM Octo PASS, post-Octo color ACCEPT, `git diff --check`, and `make test`; KASREL-003 has implementation/test/no-write smoke evidence from KAH run `run-20260608T135736Z-12a865a6fc30` and remains In Progress pending final review/KAH final gate; KASREL-004 remains planned for guidance work.
- KAB alignment requires a separately assigned KAB docs/update task.

## Next record action

INITDOC, BOOTSTRAP, `CLIMVP-001` through `CLIMVP-005`, `GRAPHMVP-001` through `GRAPHMVP-004`, `STALECLEAN-001` through `STALECLEAN-004`, `KABADOPT-001` through `KABADOPT-003`, and KASUPD-001 through KASUPD-003 are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. KABADOPT-004 remains `Planned` until its own Stage 2 pilot evidence/review gates pass. KASUPD-004 remains `Planned` until its own write-capable sync/pilot evidence gates pass. KASREL-001 is `Completed` as the accepted release-compatibility contract; KASREL-002 is `Completed` and pre-commit-ready after implementation/tests, Gray fix acceptance, official KAB GLM Octo PASS, post-Octo color ACCEPT, `git diff --check`, and `make test`; final KAH gate and commit approval remain separate pre-commit gates. KASREL-003 is `In Progress` with implementation/test/no-write smoke evidence pending final review/KAH final gate, and KASREL-004 remains `Planned` until its skill-guidance evidence gates pass. No further STALECLEAN task is listed; any epic-level closure needs a dedicated review/update task.
