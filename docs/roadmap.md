...
INITDOC and BOOTSTRAP are closed. `CLIMVP-001`, `CLIMVP-002`, `CLIMVP-003`, and `CLIMVP-004` are completed with the Go module path `github.com/SeventeenthEarth/kkachi-hermes-skills`. The next work item is `CLIMVP-005`: implement profile/project `doctor` unless 주군 chooses a different implementation slice.
