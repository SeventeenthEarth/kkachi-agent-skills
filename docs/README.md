...
- `sot/multi-agent-review-policy.md` records the accepted MAR design as KAS repository-planning authority while preserving implementation separation: local `kkachi-multi-agent-review` and `mar.py` fixture/mock/read-only surfaces exist, but MAR replaces no installed behavior until adapter proof and any KAH MAREV gates are implemented and verified.
