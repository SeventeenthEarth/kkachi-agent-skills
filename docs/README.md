...
| `sot/token-economy-and-agent-instruction-contract.md` | Candidate token-economy and agent-instruction SOT | Candidate contract for the selected candidate 6 KAS PR + 1 dependent KAH PR workstream pending responsible-approver confirmation, covering English KAS-generated prompt/CLI/console/artifact-template output, compact console output, artifact-first details, task-class gating, `AGENTS.md` / `CLAUDE.md` management, color-team project KAS lifecycle gates, uninstall/vault-backup policy, skill slimming, and KAH mechanical gates; no implementation, unapproved profile mutation, KAB activation, or Hermes runtime fork is authorized by the SOT alone |
...
- `sot/token-economy-and-agent-instruction-contract.md` — candidate SOT for the token-economy, English KAS product-output, repo-local agent-instruction, and project KAS lifecycle workstream; records the 6 KAS PR + 1 dependent KAH PR structure, compact console/artifact-first policy, `AGENTS.md` / `CLAUDE.md` management, color-team project KAS lifecycle gates, uninstall/vault-backup policy, and mechanical KAH gate boundaries without authorizing implementation or rollout by itself
