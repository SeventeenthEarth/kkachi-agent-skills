...
| `sot/multi-agent-review-policy.md` | Candidate MAR repository-promotion SOT | Promotes the accepted output SOT for Kkachi Multi-Agent Review into KAS planning authority; records Blue-default MAR, default reviewer policy, degraded coverage semantics, premium-review approval rules, Red adjudication triggers, stale GLM Octo marker targets, and the split between docs-only `MAR-001`/`MAREV-001` and later implementation tasks |
...
- `sot/multi-agent-review-policy.md` — candidate repository-promotion SOT for MAR; records that `MAR-001` is KAS docs/SOT only, paired KAH `MAREV-001` is docs/SOT only, and actual KAS skill/script/provider or KAH gate behavior requires later implementation tasks with evidence
...
- `sot/multi-agent-review-policy.md` records the accepted MAR design as KAS repository-planning authority while preserving implementation separation: MAR replaces no installed behavior until `kkachi-multi-agent-review`, `mar.py`, adapter proof, and any KAH MAREV gates are implemented and verified.
