...
- `sot/project-specific-kas-install-contract.md` — KASPROJ-001 canonical SOT for project-specific KAS install layout; requires profile targets like `skills/<project>/<project>-<phase-or-skill>/SKILL.md`, project-prefixed skill names, full suites instead of umbrella-only installs, manifest vocabulary, and doctor severity semantics; KASPROJ-002 implements read-only dry-run planning while approved install/doctor/repair/migrate remain later KASPROJ tasks
...
- `sot/project-specific-kas-install-contract.md` is the KASPROJ-001 contract for installing separate project-specific KAS suites in one Hermes profile; generic duplicate skill names such as `kkachi-plan` are invalid for project suites, umbrella-only installs are incomplete, and later dry-run/install/doctor/repair/migrate behavior remains planned.
...
- Project-specific KAS install layout and naming are defined by `sot/project-specific-kas-install-contract.md`; KASPROJ-001 is in review/commit-ready as a docs/spec contract after local docs-contract evidence, while KASPROJ-002 dry-run planning is implemented and approved install, doctor/repair/migrate, and one operational application step remain planned under KASPROJ.
