...
- `sot/role-aware-project-suite-contract.md` — planning SOT for role-aware project KAS suites; records that Blue commander profiles may use full suites, Red/Orange/Gray reviewer/scribe profiles require selected role subsets, profile-local overlay text is not enough to justify over-installed skills, explicit suite_role selection and unknown-role fail-closed behavior are required, and KASROLE-001..004 must provide registry/install/doctor/repair-prune behavior before over-installed KAH development color profiles are treated as healthy for DAGSM/WFLOW work
...
- `sot/role-aware-project-suite-contract.md` is the KASROLE planning contract for role-aware project suite subsets: `blue_commander` may use the full source suite, reviewer/scribe roles must install only selected role skills, unknown `suite_role` values fail closed, role subset manifests/doctor/repair-prune must be implemented before Red/Orange/Gray over-installed KAH development profiles are considered healthy, and WFLOW/DAGSM implementation should not normalize the over-installed transition state.
...
- Role-aware project KAS suite subset policy is defined by `sot/role-aware-project-suite-contract.md`; KASROLE-001 is completed with color evidence recorded and post-review doc corrections applied, while KASROLE-002..004 remain planned until registry/install/doctor/repair-prune implementation and the approved cleanup or removal of over-installed KAH development reviewer/scribe profiles are evidenced.
