...
NEWMAR marker: this MARTL KAS-only scope applies to the pre-NEWMAR task-loop alignment. `docs/sot/mar-execution-realignment.md` opens a later cross-repo MAR execution realignment where KAH may own deterministic provider execution after explicit `NEWMAR` tasks, review, and approval gates.
