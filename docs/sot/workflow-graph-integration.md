# KHS workflow graph integration candidate SOT

Date: 2026-05-21
Owner: KHS workflow/policy layer
Confirming role: Responsible approver / governance evidence record
Status: planning-confirmed KHS/KAH integration SOT; `kkachi-agent-helper graph` implementation evidence present; `kah graph` alias remains candidate/unimplemented
Authority level: candidate KHS/KAH integration SOT
Scope: KHS docs only; no KAH code, KAB docs, runtime configs, profiles, registries, or gateway changes
...
`.kkachi-workflow.yaml` is the canonical project workflow graph file when initialized, validated, or applied through capability-checked KAH graph evidence. `.kkachi/runs/<run_id>/phase-plan.yaml` remains run-local execution state/evidence for a KHS run, derived from or checked against the project graph and KHS phase policy when graph-managed workflow is selected.

## Layer ownership

| Layer | Owns | Must not own |
|---|---|---|
| KHS | graph templates, policy selection, phase applicability/order, declarative proposal content, run evidence requirements | deterministic graph apply/audit mechanics, direct fallback mutation |
...
|---|---|---|---|
| `.kkachi-workflow.yaml` | Project-level workflow graph instance | KHS proposes policy/templates; KAH validates/writes/applies | Project graph SOT when backed by KAH init/validation/apply evidence; never a KHS direct-edit fallback |
...
## Required KHS behavior

- KHS chooses a source template or drafts declarative graph proposal content.
- KHS first checks effective KAH capabilities for graph support. If graph support is missing from the effective binary, KHS records a roadmap/feedback gap instead of pretending the command exists.
- KHS uses `kkachi-agent-helper graph` commands only when available and capability-checked. `kah graph` remains shorthand/candidate alias text until a real alias is separately advertised.
- KHS refuses silent direct YAML edit fallback. Direct edits to `.kkachi-workflow.yaml` are unmanaged input until KAH validates or repairs them through proposal/apply evidence.
- KHS uses `init --from-template`, not `init --profile`.
- KHS must not ask KAH to execute imperative policy mutations such as `gate set`, `review-policy set`, or `graph set-policy`.
- KHS preserves graph evidence in run artifacts when graph changes affect a run.
...
Forbidden as normal KHS guidance:
...
Policy mutation category is empty. KHS and responsible approvers own policy decisions; KAH validates and records state.
...
When KHS proposes, initializes, validates, or applies graph state, the run artifacts or final report must preserve:
...
- `phase-plan.yaml` is the run-local KHS workflow/execution state SOT for one run.
- KHS may derive or constrain a run's phase plan from the project graph after capability-checked graph support is selected.
- A conflict between project graph, KHS phase policy, and `phase-plan.yaml` must stop the run until the responsible role confirms the intended authority.

## Kkachi v2 namespace collision

KHS must not treat Kkachi v2 `.kkachi/config/workflows/templates/*.json` or `.kkachi/config/workflows/addons/*.json` as KAH/KHS graph policy. They are out of this graph scope and have no fallback/merge relationship with `.kkachi-workflow.yaml`.
...
| MF-5 | Command classification contains zero policy-mutation commands; KHS must not ask KAH for imperative policy-setting graph commands. |

## Stale/conflict markers

- Any older KHS wording that calls `phase-plan.yaml` the entire workflow SOT is narrowed to run-local execution state/evidence.
...
- Exact KHS template registry format for graph generation remains a roadmap task.
- Exact KHS mapping of KAH proposal paths, event ids, and checksum/version evidence into run artifacts remains KHS integration work; KAH graph proposal/apply surfaces are implemented.
- KAB alignment with applied graph version is future/non-authoritative until a separate KAB docs update is assigned.

## Next record action

After responsible approver review closure, implement remaining KHS graph integration one PR-candidate task at a time from `docs/roadmap.md`, starting with template registry and capability-checked command guidance for the evidenced `kkachi-agent-helper graph` surface.
