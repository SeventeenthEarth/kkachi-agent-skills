...
- Any Codex or KAB-authored plan for KAS/KAH development must include a fallback audit note before Blue+Red+Orange plan review. The note must identify proposed fallback paths, explain whether each is necessary, and prefer fail-closed behavior when the system lacks capability, evidence, approval, or safe state.
- Blue, Red, and Orange plan review, first color review, MAR review when run, and post-review closure must explicitly check for unnecessary fallback paths. The normal requested change is to remove the fallback and fail closed with operator-readable diagnostics and KAH/KAB evidence.
