...
Canonical installed target layout includes both role-selected project phase skills and deterministic project composition files:

```text
~/.hermes/profiles/<profile>/skills/<project>/<project>-<phase-or-skill>/SKILL.md
~/.hermes/profiles/<profile>/skills/<project>/<project>-wrapper/SKILL.md
~/.hermes/profiles/<profile>/skills/<project>/<project>-overlay/SKILL.md
~/.hermes/profiles/<profile>/skills/<project>/<project>-overlay/references/legacy-delta-extract.md
```

Relative to a Hermes profile, the canonical target paths are:
...
skills/<project>/<project>-overlay/references/legacy-delta-extract.md
```

The `<project>` segment is the concrete project suite id, not the upstream KAS
repository name. Role-selected skill directory names must begin with the same
project id. For example, `doksuri-server-plan` is valid for the
`doksuri-server` suite; generic names such as `kkachi-plan`, `kas-plan`, or
plain `plan` are invalid as installed project-suite skill names. The wrapper and
overlay are also project-prefixed and are recorded as composition files, not as
role-selected phase skills.
...
| `installed_skill` | Project-prefixed role-selected phase/operation skill id installed under the project suite, for example `doksuri-server-plan`; generic ids are invalid for project suites. |
| `composition_files` | Deterministic wrapper/overlay/reference files generated by `install --project`; these are KAS-owned profile-local composition surfaces and are not counted as role-selected phase skills. |
| `target_path` | Profile-relative canonical path for the installed skill or composition file, for example `skills/doksuri-server/doksuri-server-plan/SKILL.md`, `skills/doksuri-server/doksuri-server-wrapper/SKILL.md`, or `skills/doksuri-server/doksuri-server-overlay/references/legacy-delta-extract.md`. |
      ...
      "composition_files": [
        {
          ...
          "name": "doksuri-server-wrapper",
          "target_path": "skills/doksuri-server/doksuri-server-wrapper/SKILL.md",
          "checksum": "sha256:<hex>",
          "backup_relative_path": null,
          "drift_policy": "manual_review_required",
          "tailoring_mode": "generated_project_wrapper"
        },
        {
          "kind": "project_overlay",
          "name": "doksuri-server-overlay",
          "target_path": "skills/doksuri-server/doksuri-server-overlay/SKILL.md",
          "checksum": "sha256:<hex>",
          "backup_relative_path": null,
          "drift_policy": "manual_review_required",
          "tailoring_mode": "generated_project_overlay"
        },
        {
          "kind": "project_overlay_reference",
          "name": "doksuri-server-overlay-legacy-delta-extract",
          "target_path": "skills/doksuri-server/doksuri-server-overlay/references/legacy-delta-extract.md",
          "checksum": "sha256:<hex>",
          "backup_relative_path": null,
          "drift_policy": "semantic_port_required_when_legacy_exists",
          "tailoring_mode": "generated_project_overlay_reference"
        }
      ]
    }
  ]
...
| unknown profile skill dir | `warning` | A profile skill directory not tracked by the KAS manifest is present. It must not be silently adopted, overwritten, or deleted. Escalate to `error` only if it shadows a required project-prefixed skill or makes identity ambiguous. Canonical wrapper/overlay directories recorded in `composition_files[]` are tracked composition surfaces and must not be reported as unknown. |
...
KASPROJ-004 implemented doctor/repair forms; public migrate forms are removed:
...
When `doctor --project-suite` is absent, `doctor --project <path>` keeps its existing KAH project-path meaning. When `--project-suite` is present, `--project` is the project suite id. `repair-project-kas` defaults `source_pack` to `kas-default-project-suite`; optional explicit `--source-pack` values fail closed when unknown, and the resolved source pack is included in JSON, diagnostics, and plan-hash evidence. Public `migrate-project-kas` is removed to avoid implying deterministic CLI semantic migration.

`KASPROJ-004` doctor/repair reports the severities above; repair starts with dry-run evidence, requires explicit approval before profile mutation, creates backups for changed files and previous manifests, preserves project-tailored language/characteristics, adopts existing canonical overlay/reference composition files into `composition_files[]` by recording their current checksums, repairs the generated wrapper from the canonical template when needed, and never mutates auth, tokens, secrets, gateway, provider/model config, KAH state, or KAB runtime state.
...
Transition from older generic/global installs is explicit through `install --from-generic` planning or skill-mediated review. The CLI must not guess project language or authority content silently. If project-specific content cannot be generated safely, it produces or records manual semantic-port work (`manual_semantic_port_tasks[]` where JSON applies) rather than installing a generic fallback. KASPROJ-004 retains generic files; deletion or de-manifest policy is deferred unless separately approved.
...
- `docs/sot/kas-cli-contract.md` owns the existing CLIMVP profile-scoped
  ...
  project-suite install, KASPROJ-004 project-suite doctor/repair,
...
this docs/spec PR is satisfied. Approved install, doctor, and repair
work must remain under KASPROJ-003 through KASPROJ-004 or later explicit tasks; semantic migration remains skill-mediated, not a public CLI surface.
