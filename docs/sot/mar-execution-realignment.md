...
| Task ID | Owner | Status | Acceptance criteria | Evidence and review gates |
|---|---|---|---|---|
| V01CLEAN-001 | KAS | Completed source-side; commit pending push | Removed v0.1.x active skills/operator guidance and converted KAS Python/shell/interpreter-wrapper MAR surfaces to fail-closed diagnostics or negative-test contexts. | Evidence: KAS commit `4dd9ccc`; tests and gates recorded in the V01CLEAN-001 run. Push/install/profile cleanup remain separate approvals. |
| V01CLEAN-002 | KAH | Completed source-side; commit pending push | KAH Go-native `go_native_argv` is the only supported MAR execution target; Python/shell/legacy adapter/interpreter-wrapper paths fail closed before spawn. | Evidence: KAH commit `ad436b9`; tests, MAR, second-color review, and final gate recorded in the V01CLEAN-002 run. Push/install/release/runtime rollout remain separate approvals. |
| V01CLEAN-003 | KAS+KAH | Completed source-side; commit pending push | Cross-repo docs/skills closeout so v0.2 is the only active path and all remaining legacy references are fail-closed/negative-test/historical only. | KAS-hosted run `run-20260704T044016Z-608a1b04b8b5`; implementation approved by 주군; KAS/KAH tests, focused Red/Orange/Gray re-review, Blue synthesis, docs-only MAR N/A evidence, KAH final gate `evt-006617`, and final phase-plan validation PASS. Push/install/release/runtime rollout remain separate approvals. |
