...
Authority level: source of truth for future KAS release-compatibility provenance and dependency audit fields
Scope: `kkachi-hermes-skills` KAS provenance/dependency contract for future `list --json`, `install --dry-run --json`, and `doctor --json` output only
...
- the evidence inputs and precedence rules KAS must use for future classification;
- deleted-bundle, shadowing, ambiguity, and dependency diagnostics; and
- future JSON fields for `list --json`, `install --dry-run --json`, and `doctor --json`.
...
- KASREL-002, KASREL-003, or KASREL-004 implementation; or
- fallback lookup paths for deleted Hermes bundle skills.

Future implementation tasks must preserve the KAS/KAH/KAB split: KAS owns policy, contracts, skill-pack provenance, and user-facing guidance; KAH owns deterministic project state, artifacts, schemas, events, locks, diagnostics, and gates; KAB owns backend runtime/session control and bridge execution evidence.
...
Future KASREL JSON output must use only these source classes unless a later accepted SOT revises this list.
...
Future KAS provenance classification must use source evidence from these inputs when available:
...
When future implementation classifies an effective skill, it must apply these precedence and diagnostic rules:
...
Future KAS provenance output must report shadowing or ambiguity when evidence shows any of these conditions:
...
Ambiguity must fail closed as `unknown_or_unclassified` unless future implementation has enough non-secret evidence to prove a canonical source class. Diagnostics should identify the ambiguous source evidence without leaking secrets.
...
`skill_dependencies` describe named skills. They may include KAS pack names, external skill names, or Hermes skill names when a skill truly depends on another skill as a skill. A missing required skill dependency is a provenance/dependency diagnostic and may fail future readiness checks closed.
...
## 9. Future JSON fields

The fields in this section are future-facing contract fields for KASREL-002 and KASREL-003 implementation. They are not current CLI output promises until the implementation tasks are completed and verified.
...
### 9.1 Common future top-level fields

Future `list --json`, `install --dry-run --json`, and `doctor --json` outputs should include a provenance contract version and inventory summary when provenance auditing is in scope:
...
### 9.2 Future per-skill or per-pack fields

Future per-skill or per-pack records should include provenance, diagnostics, shadowing, deleted-bundle, and dependency fields where relevant:
...
### 9.3 Future dependency fields

Future dependency records should preserve the taxonomy split:
...
### 9.4 `list --json` future additions

Future `list --json` records should add these fields without removing the existing CLIMVP output contract until a separately accepted compatibility decision says otherwise:
...
### 9.5 `install --dry-run --json` future additions

Future `install --dry-run --json` records should include no-write provenance evidence that can be bound into the existing approval hash when later implementation authorizes it:
...
### 9.6 `doctor --json` future additions

Future `doctor --json` records should include audit summaries, per-installed-pack provenance fields, and diagnostics:
...
## 10. Verification and follow-on task boundaries
